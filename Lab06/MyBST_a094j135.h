...
private:
    ...
    void rotateLeft(BinaryNode *&t)
    {
        ...
        tmp->height = 1 + std::max(height(tmp->left), height(t));
        t = tmp;
        // code ends
    }
    ...
    void rotateRight(BinaryNode *&t)
    {
        ...
        tmp->left = t;
        t->height = 1 + std::max(height(t->left), height(t->right));
        tmp->height = 1 + std::max(height(t), height(tmp->right));
        t = tmp;
        // code ends
    }
...
public:
    ...
    const ComparableType &findMin() const
    {
        ...
        if (empty())
        {
            throw std::underflow_error("Minimum data element not found.");
        }
    ...
    const ComparableType &findMax() const
    {
        ...
        if (empty())
        {
            throw std::underflow_error("Maximum data elemenot not found.");
        }
    ...
    bool lowestCommonAncestor(const ComparableType &x, const ComparableType &y, ComparableType &lca)
    {
        ...
        while (current != nullptr)
        {
            ...
            else if (contains(x, current) && contains(y, current))
            {
                lca = current->element;
                return true;
            }
